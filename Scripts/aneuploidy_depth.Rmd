...
## Create 5 windows per chromosome
```{r}
full_phased_snps[, SEGMENT := cut(1:.N, breaks = 5, labels = FALSE), by = c("SOURCE", "CHROM", "Haplotype")]
...
aneu_dp_equal <- small_table %>%
  ...
  geom_tile(color = NA) +
  scale_fill_gradientn(colors = c("royalblue1", "royalblue", "grey95", "grey94", "orangered", "orangered2")) + 
...
ggsave("./Figures/Aneuploidy_Depth/depth_in_5_windows_per_chr.pdf", height = 3, width = 6.18)
ggsave("./Figures/Aneuploidy_Depth/depth_in_5_windows_per_chr.png", height = 3, width = 6.18)
...
ggsave("./Figures/Aneuploidy_Depth/depth_in_5_windows_per_chr_legend.pdf", height = 3, width = 6.24)
ggsave("./Figures/Aneuploidy_Depth/depth_in_5_windows_per_chr_legend.png", height = 3, width = 6.24)
```
...
full_phased_snps %>%
  ...
  theme(legend.position = c(0.88,0.8),
        ...
        plot.margin = margin(t = 0.0,r = 0.0,b = 0.0,l = 0.01, unit = "cm"))
...
full_phased_snps %>%
  ...
  theme(legend.position = c(0.88,0.8),
        legend.key.size = unit(0.2, units = "cm"),
        plot.margin = margin(t = 0.0,r = 0,b = 0.0,l = 0.0, unit = "cm")
  )
...
None of these look great. The best option so far is to pick one chromsome from 1 sample as an example
```{r}
full_phased_snps %>%
  filter(CHROM %in% 4) %>%
