...
phased_snps <- phased_snps %>%
  mutate(Haplotype = if_else(Variant_is_hap1 == TRUE, "hap1", "hap2"))

phased_snps_small <- phased_snps %>%
  select(CHROM, POS, VAF, Haplotype, SOURCE)
```

## Create the reciprocal dataset
I am only worrying about VAF now, but I will need to do this with a more sophisticated analysis later.
This seems to have messed something up.
```{r}
flip_phased_snps_small <- phased_snps_small %>%
  mutate(Haplotype = ifelse(Haplotype == "hap1", "hap2", "hap1")) %>%
  mutate(VAF = 1 - VAF)
...
```{r}
sum_vaf$Haplotype <- gsub("hap1", "A", sum_vaf$Haplotype)
sum_vaf$Haplotype <- gsub("hap2", "B", sum_vaf$Haplotype)
```
...
```{r}
setDT(phased_snps)
phased_snps[, SEGMENT := cut(1:.N, breaks = 100, labels = FALSE), by = c("SOURCE", "CHROM", "Haplotype")]
```
