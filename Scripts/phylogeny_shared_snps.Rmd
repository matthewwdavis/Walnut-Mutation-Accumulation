...
unique_shared_snps <- function(snp.matrix, samples){
 ...
 # Create a dataframe from the matrix
  ...
  # Create a list of dfs specifying samples
    ...
    data_list[[sample]] <- long_df_data %>% filter(Sample == sample)
  }
  ...
  unique_shared.snps <- variable_snps %>%
    filter(!(Site %in% other_snps$Site))
  
    return(unique_shared.snps)
}
```
...
## Embryo Subclade 2.1
test <- unique_shared_snps(snp.matrix30,  c("cr2_15A", "cr2_15A1", "cr2_18A"))
