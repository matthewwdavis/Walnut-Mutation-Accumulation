...
group_info <- function(data){
  ...
  table <- data %>%
    ...
    mutate(Years = case_when(
      Source %in% c("ref_chandler", "tree2") ~ NA,
      Source %in% c("cr2_hifi", "cr21_1", "cr21_2", "cr2_11A", "cr2_12A", "cr2_13A", "cr2_15A", "cr2_15A1", "cr2_16A", "cr2_16A1",
...
sum.table <- sample_sums.df %>%
  ...
  summarise(mean_mut = mean(count),
            ...
            lower_ci = quantile(count, 0.005),
            upper_ci = quantile(count, 0.995))
...
## Remove uninformative columns to save RAM
```{r}
