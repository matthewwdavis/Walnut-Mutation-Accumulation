...
bootstrap_results <- bootstrap_by_source(dn.df_20, source_col = "Source", prob_col = "PROB", R = 100)
...
weighted_bootstrap <- function(data, R = 10){
  ...
  for (i in seq_len(R)) {
    bootstrap_sample <- unique(data[sample(nrow(data), size = nrow(data), replace = TRUE, prob = data$PROB)])
...
weighted_bootstrap <- function(data, R = 100) {
...
bootstraps <- weighted_bootstrap(dn.df_20)
...
## Try splitting by source
```{r}
weighted_bootstrap <- function(data, R = 10) {
  # Split the data by the Source column
  split_data <- split(data, by = "Source")
  ...
  # Loop to create R bootstrap samples
  for (i in seq_len(R)) {
    # Initialize an empty list to store the sampled subsets
    sampled_subsets <- list()
    
    # Sample from each subset
    for (source in names(split_data)) {
      subset <- split_data[[source]]
      sampled_subset <- unique(subset[sample(.N, size = .N, replace = TRUE, prob = subset$PROB)])
      sampled_subsets[[source]] <- sampled_subset
    }
    
    # Combine the sampled subsets into a single bootstrap sample
    bootstrap_sample <- do.call(rbind, sampled_subsets)
    bootstrap_samples[[i]] <- bootstrap_sample
    
    # Count the number of rows for each source in the bootstrap sample
    source_counts[[i]] <- table(bootstrap_sample$Source)
  }
...
split_bootstraps <- weighted_bootstrap(dn.df_20)

source_counts2 <- split_bootstraps$source_counts

source_counts_df2 <- do.call(rbind, source_counts2)

table2 <- as.data.table(source_counts_df2) %>%
  pivot_longer(cols = everything())

table2$name <- factor(table2$name, levels = levels)
