...
# Add informative grouping columns for plotting
group_info <- function(data){
  ...
  table <- data %>%
    ...
    mutate(Years = case_when(
      Source %in% c("ref_chandler", "tree2") ~ 0,
      Source %in% c("cr2_hifi", "cr21_1", "cr21_2", "cr2_11A", "cr2_12A", "cr2_13A", "cr2_15A", "cr2_15A1", "cr2_16A", "cr2_16A1",
                  "cr2_16A2", "cr2_17A1", "cr2_17A2", "cr2_18A", "cr2_18A1") ~ 2022-1995,
      Source %in% "cr85" ~ 2023-1985,
      Source %in% "cr10" ~ 2023-2010,
      Source %in% "cr13" ~ 2023-2013,
      Source %in% "cr22" ~ 2023-2022)) %>%
    mutate(Genome_Size = case_when(
      Source %in% c("ref_chandler", "tree2", "cr2_hifi", "cr21_1", "cr21_2", "cr2_11A", "cr2_12A", "cr2_13A", "cr2_15A", "cr2_15A1", "cr2_16A", "cr2_16A1",
                  "cr2_16A2", "cr2_17A1", "cr2_17A2", "cr2_18A", "cr2_18A1", "cr85", "cr10", "cr13", "cr22") ~ 540705719)) %>%
    mutate(Embryogenic = case_when(
      Source %in% c("ref_chandler", "tree2", "cr85", "cr10", "cr13", "cr22") ~ 0,
      Source %in% c("cr2_hifi", "cr21_1", "cr21_2", "cr2_11A", "cr2_12A", "cr2_13A", "cr2_15A",
                    "cr2_15A1", "cr2_16A", "cr2_16A1", "cr2_16A2", "cr2_17A1", "cr2_17A2",
                    "cr2_18A", "cr2_18A1") ~ 1
    ))
...
# Function to bootstrap snps
bs_snps <- function(data, samples = 100){
  # Create and empty vector
  result_list <- c()
  # loop
  for (i in 1:samples) {
    # sample each SNP based on probability
    data$Select <- sapply(data$PROB, function(prob) rbinom(n = 1, size = 1, prob = prob))
    # save each dataframe
    result_list[[i]] <- data
    }
  return(result_list)
...
bs_summary <- function(data, samples = 100){
  # Create an empty vector
  summary_list <- c()
  # loop through to summarize each table
  for (i in 1:samples) {
    summary <- data[[i]] %>%
      group_by(Source) %>%
      filter(Select == 1) %>%
...
# Bootstrap the snps to identify mutations
## Read in and prep data
```{r}
# Read in the data
dn_snps.df <- fread("./Data/VCFs/Filtered/dn_snps_no_share.vcf")

# Add probability based off of QUAL score
dn_snps.df <- dn_snps.df %>%
...
# Add necessary grouping information
dn_snps.df <- group_info(dn_snps.df)

# Identify levels for later plotting
...
# Test by placing a filter on QUAL >= 30
 # dn_snps.df <- dn_snps.df %>%
 #   filter(QUAL >= 30)
```


## Sampling with a binomial, bootstrapping, checking distribution, and preparing plotting tables
```{r}
set.seed(25)
# Only keep necessary information for more bootstraps
snps_bs.df <- dn_snps.df %>%
  select(Source, unique, PROB)

# Bootstrap and summarize
sample.list <- bs_snps(snps_bs.df, samples = 5000)
sample.sums <- bs_summary(sample.list, samples = 5000) 

# Create necessary tables
sample_sums.df <- do.call(rbind, sample.sums)
...
sum.table <- sample_sums.df %>%
  group_by(Source) %>%
  summarise(mean_mut = mean(count),
            median_mut = median(count),
            num_bs = n(),
            stand_dev = sd(count),
            stand_dev_pop = sd(count)*sqrt((num_bs-1)/num_bs),
            lower_ci99 = quantile(count, 0.005),
            upper_ci99 = quantile(count, 0.995),
            lower_ci95 = quantile(count, 0.025),
            upper_ci95 = quantile(count, 0.975))

# Add designating groups to samples
sum.table <- group_info(sum.table)
sample_sums.df <- group_info(sample_sums.df)

# Set levels for plotting
sum.table$Source <- factor(sum.table$Source, levels = levels)
sample_sums.df$Source <- factor(sample_sums.df$Source, levels = levels)
```

## Testing model
```{r}
shoot.table <- sum.table %>%
  filter(Embryogenic == 0, Years > 0)

mod1 <- lm(data = sum.table, formula = mean_mut ~ Years)
summary(mod1)

mod2 <- lm(data = sum.table, formula = mean_mut ~ Years + Embryogenic)
summary(mod2)

mod3 <- lm(data = shoot.table, formula = mean_mut ~ Years)
summary(mod3)
```

## Plot bootstrap results percentile confidence intervals
```{r}
sum.table %>%
  ggplot(aes(x = Source, y = mean_mut)) +
  geom_errorbar(aes(ymin = lower_ci99, ymax = upper_ci99), width = 0.5) +
  geom_point(aes(color = Organism)) +
  #geom_text(aes(label = round(mean_value, 0)), vjust = -0.5, size = 2.5) +  # Add this line for the labels
...
ggsave("./Figures/SNP_Calling/dn_snp_bootstrap_n5000_ci99.pdf", height = 2, width = 5)
ggsave("./Figures/SNP_Calling/dn_snp_bootstrap_n5000_ci99.png", height = 2, width = 5)
...
sample_sums.df %>%
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1),
        ...
        strip.text = element_text(face = "bold"),
        legend.position = "none")
ggsave("./Figures/SNP_Calling/dn_snp_bootstrap_n5000_violin.pdf", height = 2, width = 5)
ggsave("./Figures/SNP_Calling/dn_snp_bootstrap_n5000_violin.png", height = 2, width = 5)
# Branching events give rise to higher probability of detecting mutations
# 50% allele frequency
```

# Calculate rates with the bootstrapped data
## Plot mutation rate based on bootstrap
```{r}
# Shoot mutation rates
sum.table %>% 
  filter(Source %in% c("cr85", "cr10", "cr13", "cr22")) %>%
  ggplot(aes(x = Years, y = mean_mut)) +
  geom_errorbar(aes(ymin = lower_ci99, ymax = upper_ci99), width = 1.5) +
  geom_point(aes(color = Organism)) +
  geom_smooth(method = "lm", se = FALSE, color = "red3") +
  scale_color_manual(values = "dodgerblue") +
  labs(title = "Shoot culture mutation over time", 
         x = "Years in tissue culture", y = "Mean Number of Mutations") +
  theme_classic(base_size = 6) +
  theme(plot.title = element_text(hjust = .5, face = "bold"),
        ...
        strip.text = element_text(face = "bold"),
        legend.position = "none")
ggsave("./Figures/SNP_Calling/shoot_dn_snp_mut_year_bs5000_ci99.pdf", height = 2, width = 3)
ggsave("./Figures/SNP_Calling/shoot_dn_snp_mut_year_bs5000_ci99.png", height = 2, width = 3)
```

## Calculate the mutation rate of each individual
```{r}
# Calculate mutation rates and CI per individual
mut_rate_ind.df <- sample_sums.df %>%
  mutate(Mut_Rate_BP = count / Genome_Size) %>%
  mutate(Mut_Rate_BP_Year = count / Genome_Size / Years) %>%
  group_by(Source) %>%
  summarise(mean_mut_rate_bp = mean(Mut_Rate_BP),
            mean_mut_rate_bp_year = mean(Mut_Rate_BP_Year),
            lower_ci99_bp = quantile(Mut_Rate_BP, 0.005, na.rm = TRUE),
            upper_ci99_bp = quantile(Mut_Rate_BP, 0.995, na.rm = TRUE),
            lower_ci99_bp_year = quantile(Mut_Rate_BP_Year, 0.005, na.rm = TRUE),
            upper_ci99_bp_year = quantile(Mut_Rate_BP_Year, 0.995, na.rm = TRUE),
            lower_ci95_bp = quantile(Mut_Rate_BP, 0.025, na.rm = TRUE),
            upper_ci95_bp = quantile(Mut_Rate_BP, 0.975, na.rm = TRUE),
            lower_ci95_bp_year = quantile(Mut_Rate_BP_Year, 0.025, na.rm = TRUE),
            upper_ci95_bp_year = quantile(Mut_Rate_BP_Year, 0.975, na.rm = TRUE))

# Add grouping information
mut_rate_ind.df <- group_info(mut_rate_ind.df)

# Plot individual mutation rates
mut_rate_ind.df %>%
  ggplot(aes(x = Source, y = mean_mut_rate_bp)) +
  geom_point(aes(color = Organism)) +
  geom_errorbar(aes(ymin = lower_ci99_bp, ymax = upper_ci99_bp), width = 0.5) +
  #geom_text(aes(label = round(mean_value, 0)), vjust = -0.5, size = 2.5) +  # Add this line for the labels
  facet_grid(~Type, scales = "free", space = "free") +
  scale_color_manual(values = c("Tree" = "seagreen", 
                               "Shoot" = "dodgerblue", 
  ...
  scale_x_discrete(labels = c("ref_chandler" = "Reference Tree", "tree2" = "Tree 2",
                              ...
                              "cr85" = "Chandler '85", "cr10" = "Chandler '10", "cr13" = "Chandler '13",
                              "cr22" = "Chandler '22")) +
  labs(y = "Mean Mutations per Base Pair")+
  theme_classic(base_size = 6) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1),
        axis.title.x = element_blank(),
        strip.background = element_blank(),
        strip.text = element_text(face = "bold"),
        legend.position = "none")
# Lower bar would be the higher per year mutation rate. Chandler 22 is a bit odd, but only 1 year in culture.
```

## Look at Mutation Rates by Organism
```{r}
# Create a data table on a per organism basis
mut_rate_org.df <- sample_sums.df %>%
  mutate(Mut_Rate_BP = count / Genome_Size) %>%
  mutate(Mut_Rate_BP_Year = count / Genome_Size / Years) %>%
  group_by(Organism) %>%
  summarise(mean_mut_rate_bp = mean(Mut_Rate_BP),
...
mut_rate_org.df %>%
  filter(!Organism %in% "Tree") %>%
  ggplot(aes(x = Organism, y = mean_mut_rate_bp_year, fill = Organism)) +
  geom_bar(stat = "identity") +
  geom_errorbar(aes(ymin = lower_ci99_bp_year, ymax = upper_ci99_bp_year), width = 0.5) +
  #geom_text(aes(label = round(mean_value, 0)), vjust = -0.5, size = 2.5) +  # Add this line for the labels
...
## Remove uninformative columns to save RAM and read in genome data
```{r}
# Remove unneeded columns from de novo SNPs
all_dn.df <- dn_snps.df %>%
  select(-c(ID, FILTER, INFO, FORMAT, Hap, default, PL))
...
# Make VAF plot function
```{r}
vaf_plot <- function(vcf_data, organisms = c("Embryo", "Tree", "Shoot"), qualfilt = 30, linewide = 1, legendsize = 1, treecolor = "darkseagreen4", shootcolor = "darkseagreen", embryocolor = "goldenrod1"){
  
  plot <- vcf_data %>%
    filter(Organism %in% organisms,
           median_QUAL >= qualfilt) %>%
    ggplot(aes(x = VAF, color = Organism)) +
...
```{r}
all_dn.df %>%
  filter(Organism == "Shoot", median_QUAL >= 30) %>%
...
all_dn.df %>%
  ...
  ggplot(aes(x = VAF, fill = Organism)) +
...
vaf_plot(all_dn.df)
ggsave("./Figures/SNP_Calling/vaf_density_all_q30.pdf", height = 2, width = 3)
ggsave("./Figures/SNP_Calling/vaf_density_all_q30.png", height = 2, width = 3)

vaf_plot(all_dn.df, organisms = c("Shoot", "Embryo"))
ggsave("./Figures/SNP_Calling/vaf_density_shoots_vs_embryos_q30.pdf", height = 2, width = 3)
ggsave("./Figures/SNP_Calling/vaf_density_shoots_vs_embryos_q30.png", height = 2, width = 3)

vaf_plot(all_dn.df, organisms = c("Tree", "Embryo"))
ggsave("./Figures/SNP_Calling/vaf_density_trees_vs_embryos_q30.pdf", height = 2, width = 3)
ggsave("./Figures/SNP_Calling/vaf_density_trees_vs_embryos_q30.png", height = 2, width = 3)

vaf_plot(all_dn.df, organisms = c("Tree", "Shoot"))
ggsave("./Figures/SNP_Calling/vaf_density_trees_vs_shoots_q30.pdf", height = 2, width = 3)
ggsave("./Figures/SNP_Calling/vaf_density_trees_vs_shoots_q30.png", height = 2, width = 3)

vaf_plot(all_dn.df, organisms = "Embryo")
ggsave("./Figures/SNP_Calling/vaf_density_embryos_q30.pdf", height = 2, width = 3)
ggsave("./Figures/SNP_Calling/vaf_density_embryos_q30.png", height = 2, width = 3)

all_dn.df %>%
  filter(median_QUAL >= 30) %>%
  ggplot(aes(x = norm_DP, y = VAF)) +
