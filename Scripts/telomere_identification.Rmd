...
bp_correction <- function(df, name, bp_number){
  new_df <- df %>% 
...
ggplot(data = read_telomeres_cor_long_bp, aes(x = ID, fill = ID, y = Corrected_Count)) +
  ...
  labs(y = "Number of 21-mer telomere sequences in nuclear long reads\n(corrected with total bp/21)", x = "") +
  coord_flip() +
  theme_classic() + 
  theme(plot.title = element_text(hjust = 0.5), legend.position = "none", axis.text.x = element_text(angle = 45, hjust = 1))
#ggsave("~/Projects/Walnut/Longterm_Tissue_Culture/Figures/Bar_Charts/Telomeres/long_read_telomere_counts_nuclear_reads_long.png", height = 3, width = 6)
#ggsave("~/Projects/Walnut/Longterm_Tissue_Culture/Figures/Bar_Charts/Telomeres/long_read_telomere_counts_nuclear_reads_long.pdf", height = 3, width = 6)
