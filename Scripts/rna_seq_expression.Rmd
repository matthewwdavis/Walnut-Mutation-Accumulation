...
all_counts %>%
  filter(tpm > 1.0) %>%
  ggplot(aes(x = as.factor(seq_id), y = log(tpm), fill = Group)) + 
  geom_boxplot() +
  facet_wrap(~Source, scales = "free") +
  scale_fill_manual(values = c("chr_4" = "red", "chr_9" = "red", "Other" = "gray")) +
...
all_counts %>%
  filter(tpm > 1.0) %>%
  ggplot(aes(x = as.factor(seq_id %in% c(4,9)), y = log(tpm), fill = Group)) + 
  geom_boxplot() +
  facet_wrap(~Source, scales = "free") +
  scale_fill_manual(values = c("chr_4" = "red", "chr_9" = "red", "Other" = "gray")) +
...
all_counts$Group <- factor(all_counts$Group, levels = grp_levels)
...
all_counts %>%
  filter(tpm > 1.0) %>%
  ggplot(aes(x = Group, y = log(tpm), fill = Group)) + 
  geom_boxplot() +
  facet_wrap(~ Source, scales = "free") +
