...
## Functions
```{r}
plot_exp_branch<-function(i, tpmfilt = 0, R){
  
  # Filter for minimum tpm
  all_exp_data_tmp<-all_exp_data[mintpm > tpmfilt]
...
exp_tree_tests <- function(tree_exp, all_exp_data, tpmfilt = 0, Gene_sub = NULL, random = FALSE) {
...
# I think this is breaking each chrom into 50 windows. I may want to do 10 kb windows or something like that instead
...
exp_files <- list.files("./Data/Kallisto/", full.names = TRUE)
...
#all_exp_data[, Source := names[Source]]
```
...
# Create a mean tpm column for each transcript
all_exp_data[, meantpm := mean(tpm), by = RNA]
#  Create a minimum tpm column for each transcript
