...
```{r}
mut_colors<-c(`C>A`="cyan3",`C>G`="black",`C>T`="red4",`T>A`="gray",`T>C`="green3",`T>G`="pink3")

plot_levels <- c("ref_chandler", "tree2", "cr22", "cr13", "cr10", "cr85",
...
```{r}
# Tricontext
ggplot(embryo_trimers, aes(x = TRI, y = SOURCE, fill = MUT, size = genomeprop, alpha = genomeprop)) +
  geom_point(shape = 22) +
  facet_grid(~MUT, scales = "free_x", labeller = labeller(MUT = c(
    `C>A` = "<span style = 'color: cyan3;'>C>A</span>",
    `C>G` = "<span style = 'color: black;'>C>G</span>",
    `C>T` = "<span style = 'color: red4;'>C>T</span>",
    `T>A` = "<span style = 'color: grey;'>T>A</span>",
    `T>C` = "<span style = 'color: green3;'>T>C</span>",
    `T>G` = "<span style = 'color: pink3;'>T>G</span>"
  ))) +
...
ggsave("./Figures/Tricontext/sample_individual_tricontext.pdf", height = 3.2, width = 6.5)
