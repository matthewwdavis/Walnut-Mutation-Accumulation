...
cds_good <- fread("./Data/GFFs/Parsed/cds_good.tsv")
...
## Filtering shared tree mutations
```{r}
tree_mut <- mutations_save[CULTURETYPE %in% "tree"]

tree_mut[, NUMIND := .N, by = UNIQUE]

# Plot distributions
tree_mut %>%
  filter() %>%
  group_by(CHROM) %>%
  mutate(bin = cut(POS, breaks = 50, labels = FALSE)) %>% 
  group_by(CHROM, bin) %>%
  summarise(avg_VAF = mean(VAFREAL, na.rm = TRUE), .groups = "drop") %>%
  ggplot(aes(x = bin, y = avg_VAF, fill = CHROM)) +
  geom_col() +
...
## Summarize average VAFs
tree_mut %>%
  group_by(SOURCE, NUMIND) %>%
  summarise(
    avg_vaf = mean(VAFREAL),
    num_var = n()
  )

share_tree <- tree_mut[NUMIND %in% 2, UNIQUE]
```

## Filtering shared shoot mutations
```{r}
shoot_mut <- mutations_save[CULTURETYPE %in% "shoot"]

shoot_mut[, NUMIND := .N, by = UNIQUE]
...
share_shoot <- shoot_mut[NUMIND %in% c(2,3,4), UNIQUE]
```

## Filter out shared tree and shoot mutation
```{r}
mutations_filt_save <- mutations_save[!(UNIQUE %in% share_tree)]
mutations_filt_save <- mutations_filt_save[!(UNIQUE %in% share_shoot)]
```

# Write out new files
```{r, eval=T}
fwrite(mutations_save, "./Data/VCFs/Parsed/mutations.tsv", sep = "\t")
fwrite(mutations_filt_save, "./Data/VCFs/Parsed/mutations_strict.tsv", sep = "\t")
```
